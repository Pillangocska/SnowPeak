...
class LiftView:
    def __init__(self, simulator):
        ...
        pygame.display.set_caption(f"Ski Lift Simulation - {simulator.lift.lift_id}")
    ...
    def draw_lift(self):
        ...
        for i, seat in enumerate(self.simulator.seats):
        ...
        bottom_elevation = self.font.render(f"Elevation: {self.simulator.lift.start_elevation}m", True, self.text_color)
        top_elevation = self.font.render(f"Elevation: {self.simulator.lift.end_elevation}m", True, self.text_color)
    ...
    def draw_info(self):
        info_text = [
            f"Lift ID: {self.simulator.lift.lift_id}",
            f"State: {self.simulator.lift.state.value}",
            f"Start: ({self.simulator.lift.start_lat}, {self.simulator.lift.start_lon})",
            f"End: ({self.simulator.lift.end_lat}, {self.simulator.lift.end_lon})",
            f"Total Capacity: {self.simulator.lift.num_seats} passengers",
                    ...
                    if event.button == 1:
                        for button in self.buttons:
                            if button['rect'].collidepoint(event.pos):
                                self.simulator.set_state(button['state'])
